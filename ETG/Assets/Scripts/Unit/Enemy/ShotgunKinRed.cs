...
public class ShotgunKinRed : Enemy
{
    [SerializeField]
    GameObject weapon;
    ...
    IEnumerator ShootDelay()
    {
        while (true)
        {
            ...
            if (distance <= 200)
                weapon.GetComponent<EnemyGun>().ShootReady(dir, 200, 1, 4, 40);
        }
    }
    ...
    public override void DieLogic()
    {
        ...
        if (ani.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                bullet.GetComponent<Bullet>().SetInfo(i * 90, 200, 1, Bullet.Team.Enemy);
